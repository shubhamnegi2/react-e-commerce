                                ...
                                <ul className="nav nav-pills nav-justified mb-3" id="ex1" role="tablist">
                                    {currDesc.map((item) => (
                                        (Object.keys(item.data)).map((keys) => (
                                            <li className="nav-item d-flex mx-2" role="presentation" onClick={() => showContent(keys)}>
                                ...
                                <div className="tab-content" id="ex1-content">
                                    {
                                        currDesc.map((dt) => (
                                            Object.keys(dt.data).map((key, i) => (
                                                <div className={`tab-pane ${keys == key ? "show active" : "fade"}`} >
                                                            ...
                                                            <ul className="list-unstyled mb-0">
                                                                {dt.data[key].list.length != 0 ? dt.data[key].list.map((item, idx) => (
                                                                    idx < Math.floor(dt.data[key].list.length / 2) && (
                                                                        <li key={idx}><i className="fa fa-check text-success me-2"></i>{item}</li>
                                                                    )
                                                                )) : ''}
                                                            </ul>
                                                        </div>
                                                    ...
                                                    <table className="table border mt-3 mb-2">
                                                        {dt.data[key].properties != {} ? Object.keys(dt.data[key].properties).map((prop) => (
                                                            <tr>
                                    ...
                                    <div className="card-body">
                                        ...
                                        {categoryData.slice(0, 3).map((ctg) => (
                                            <div className="d-flex mb-3" key={ctg.id}>
                                                <NavLink onClick={() => handleClick(ctg.id)} target={"_blank"} to={`/productDetails/${ctg.id}`} className="me-3">
                                                    <img src={ctg.image} style={{ minWidth: '96px', height: '96px' }} className="img-md img-thumbnail" />
                                                </NavLink >
                                                <div className="info">
                                                    <NavLink onClick={() => handleClick(ctg.id)} to={`/productDetails/${ctg.id}`} className="nav-link mb-1">
